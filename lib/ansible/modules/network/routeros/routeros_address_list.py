...
short_description: Manage entries in address lists on Mikrotik RouterOS 
description:
  - The module provides basic management actions on address list entries in 
...
options:
  address:
    description:
      - A single IP address, range of IP addresses or DNS name.
    required: true
  list:
    description:
      - Name for the address list
    required: true
  state:
    description:
...
def main():
    ...
    ctx_line = "/ip firewall address-list "

    # Lookup line is the query used to lookup and find configuration entries
    lookup_line = 'where address="' + module.params['address'] + '" list="' + module.params['list'] + '"'
    ...
    if entry_exist == 0:
      if state == "present":
        set_ip_command = ctx_line + "add address=" + module.params['address'] + " list=" + module.params['list']
      else:
